...
function createPage(
    ...
    if (scene == "") {
        // Homepage
        page = new HomePage(projects, isMobile, goTo);
        ...
        if (parts[1]) {
            ...
            if (project) {
                ...
                page = new ProjectDetailsPage(project, goTo, isMobile);
            } else {
                // NOT FOUND !! (Not implemented)
                page = new HomePage(projects, isMobile, goTo); // TEMP
            }
        } else {
            page = new ProjectPage(projects, isMobile, goTo);
        }
    } else {
